...
Given a $2$-switch
proper
\todoforkucera{Remove \quot{proper} since it's redundant.}
...
\todokucera{proper $2$-switch}
\todobartek{\quot{Proper $k$-interval} is defined like this and I'd prefer to use the expression literally where possible. In fact it means \quot{properly $k$-interval}, which doesn't make sense when we swap the words.}
$2$-interval
function $\fnba = f$.
...
and more switches}
\label{sec:3switch}
\todomaybe[inline]{Separate in a chapter, since this section deals with functions with more than $2$ intervals.}
