...
Then $\mathcal{T}$ spans $\ftwointtwostep{f}{n}{0 b}{1 a}$
if and only if
$\mathcal{T}'$ spans
...
\begin{enumerate}
\item
If $\mathcal{T}$ spans $\ftwointtwostep{f}{n}{0 b}{1 a}$,
then $\mathcal{T}'$ spans $\intervalfn{f}{n}{0 a}{1 b}$:

Let $\mathcal{T}$ span $\ftwointtwostep{f}{n}{0 b}{1 a}$.
...
&\iff c' \geq 0 a \text{ and } c' \leq 1 b \\
&\iff c \leq 0 b \text{ or } c \geq 1 a \\
&\iff \ftwointtwostep{f}{n}{0 b}{1 a}(c) = 1
\end{align*}

\end{enumerate}
\end{proof}

Lemma \ref{lemma:tflip} shows a bijection between
