...
\todokucera{Připadá mi, že když je to společné pro všechny levé/pravé hranice, tak proč značit $s_ai$ $a_bi$ s tím $i$.}
\todonote[inline]{
...
\todokucera[inline]{Pokud se na definice 5.2.[12] neodkazujete, tak je podle mne zbytečné je číslovat a dávat to do těch definic, ale stačilo by to v rámci sekce v těle textu, celá tahle podsekce je definicí.}
\end{definition}
...
It is easy to see that
the inequality \ref{ineq:difficultprefix} holds
in case $p_x$ is even.
...
collides on some false point of $f_l^n$.
...
This case is symmetric to Case \ref{item:difficultv0}.
\end{enumerate}
\end{proof}
