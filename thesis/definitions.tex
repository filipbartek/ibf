...
\end{problem}
\todo[inline]{Globally replace $INT(MIN(k))$ with $\minintk$.}
\todo[inline]{Globally remove $INT$.}
\todo[inline]{Define $MIN-SWITCH(l)$ where appropriate if necessary.}
...
\todo[inline]{Mention the previous sentence in Conclusion.}
...
\todomaybe[inline]{Leave out the following definition since we only use the special case $\switchfns{2} \cap \intervalfns{2}$.}
\begin{definition}
[$\switchfns{l}$]
\label{def:switchfns}
For any $l \geq 0$,
