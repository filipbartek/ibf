...
Notably,
$v = \round{\bit{v}{1}, \ldots, \bit{v}{n}} = \bits{v}{1}{n}$.

\begin{definition}[Concatenation $uv$]
Let $u, v$ be vectors of lengths $m, n$ respectively.
Then $uv = (u_1, \ldots, u_m, v_1, \ldots, v_n)$
is the concatenation of the vectors $u, v$.
\end{definition}

Note that we will use symbols
and vectors of length $1$ interchangeably,
for example $v = \bit{v}{1} \bit{v}{2} \ldots \bit{v}{n}$.
...
Let $S$ be an orthogonal set of size $\apply{ortho}{f}$
and $\mathcal{T}$ be any spanning set
of size $\apply{dnf}{f}$.
Since $\size{\mathcal{T}} < \size{S}$
and $\mathcal{T}$ spans $TP(f) \supseteq S$,
there must be a $T \in \mathcal{T}$ that spans
two distinct vectors $x, y \in S$.
However,
...
we may conclude that the representation is minimum.
...
and simply use Definition \label{def:coverable},
...
\todo[inline]{Define $l$-switch function?}

Table \ref{table:exampleintfns}
\todo{Fix reference.}
shows
...
\end{table}
\todo{Typeset the table more pretty?}
