...
\todomaybe{Change to $T(f)$ and $F(f)$ according to Hammer: Definition 1.1. Watch out for confusion with ternary vectors.}
...
\todo{Change from \quot{propositional} to \quot{Boolean} in the rest of the text. This is to adhere to \citet{Crama2011}.}
...
\todo{Typeset the footnote on the same page.}
\todonote[inline]{
$$
\fora{x,y \in S}{x \neq y \Rightarrow
\fora{T}{\ex{z}{\binand{f(z) = 0}{T \text{ spans } z}}}
}
$$
as opposed to
$$
\fora{x,y \in S}{x \neq y \Rightarrow
\ex{z}{\binand{f(z) = 0}{\fora{T}{T \text{ spans } z}}}
}
$$
}
...
\todo[inline]{Get rid of collision altogether (in case we don't use it in the text), or prove that it is equivalent to orthogonality, preferably using established terminology.}
\todonote[inline]{Note that $x$ and $y$ collide on $z$ if and only if $z$ separates $x$ and $y$ in terminology of \citet[Definition 3.3]{Cepek2012}. Consider using the same terminology.}
\end{definition}
...
\todokucera{Každopádně tam se používá jiná notace, ten odstavec, který máte níže a který dává do souvislosti esenciální množiny s tímhle, je potřeba dát před první odkaz na článek \citep{Cepek2012}, sem by se to docela hodilo.}
...
The orthogonal sets were studied in \citet{Cepek2012}.
...
\todokucera{pozn. definoval jste implikant?}
...
\todobartek{\citet{Cepek2012} use falsepoint essential sets. Shouldn't I refer to the result that way here?}
...
\todo[inline]{Remove the following paragraph if an introduction to the preceding theorem makes more sense.}
...
in \cref{sec:3switch}.
\todokucera[inline]{Zmiňujete v úvodu i $l$-switch funkce? To by tam mělo být.}
