...
{\definitionsource[p.~156]{Schieber2005154}}]
Let $v$ be a vector of length $n$ and $1 \leq i \leq n$.
Then $\bit{v}{i}$ is the $i$-th component of $v$.
\end{definition}
\todokucera{Možná bych to dal za D1.1.2 a řekl, že “as a special case we consider the extraction of a single component $v^{[i, i]}$ which will also be denoted as $v^{[i]}$.”}

\begin{definition}
[Subsequence extraction $\bits{v}{a}{b}$
{\definitionsource[p.~157]{Schieber2005154}}]
Let
...
that is $\bits{x}{1}{n}$.
\end{definition}
\todokucera{Tohle vypadá dost divně, když „the most significant bit (MSB) of a binary vector $x$ is its first bit, that is $x^{[1]}$”, tak logicky „n most significant bits” se dá vyložit jako $(x^{[1]})^{n}$”, navíc n používáte obvykle pro délku celého vektoru, tak by se možná hodilo jiné písmenko (třeba i, j, k, m).
Co třeba: We shall often refer to $x^{[i]}$ as to the $i$-th most significant bit and in particular we shall refer to $x^{[1]}$ as to the most significant bit of $x$. We shall often consider the first $n$ most significant bits as a vector $x^{[1,n]}$.}
...
\todomaybe{Change $v$ back to $x$ for consistence.}
...
\todokucera[inline]{\foreignlanguage{czech}{Možná bych zmínil, že \quot{T spans a binary vector x of length n, iff C(x)=1, kde C je term odpovídající T}.}}
...
{\definitionsource[p.~165]{Schieber2005154}}]
...
\begin{definition}
[Orthogonality
{\definitionsource[p.~6]{Dubovsky2012}}]
% Dubovsky2012: Definition 1.15
...
\begin{definition}
[Coverability
{\definitionsource[p.~368]{Cepek2012}}]
% Definition 2.9
\label{def:coverable}
...
\begin{definition}
[$l$-switch function
{\definitionsource[pp.~13-14]{Husek2015}}]
