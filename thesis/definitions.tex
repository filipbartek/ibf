...
\todo[inline]{Cite respective sources in definition and proof titles.
When we define an existing term, say in advance that it was defined before somewhere else.}
...
and vectors.
For example, if $v$ is a vector of length $n$,
then $00v11$ denotes a vector of length $n+4$
whose leading two components have the value $0$,
trailing two components have the value $1$
and $i$-th component for $i \in \curly{3, \ldots, n+2}$
has the value of the $(i-2)$-th component of $v$.

\begin{definition}[Component extraction $\bit{v}{i}$]
Let $v$ be a vector of length $n$ and $1 \leq i \leq n$.
Then $\bit{v}{i}$ is the $i$-th component of $v$.
\end{definition}

\begin{definition}[Subsequence extraction $\bits{v}{a}{b}$]
Let
...
\begin{definition}[Symbol repetition $\rep{\alpha}{n}$]
...
\begin{definition}[\Acrlong{msb}]
The \definiendum{\acrfull{msb}} of a binary vector $x$
is its first bit,
that is $\bit{x}{1}$.

The $n$ \definiendum{\acrlongpl{msb}} of a vector
...
\todomaybe[inline]{Wrap in table environment and add a caption.}
\begin{center}
\begin{tabular}{cc}
...
\todo[inline]{Add a section introduction.
Introduce orthogonal sets as a proof tool used in Schieber implicitly and Dubovsky explicitly.}

\begin{definition}
[Orthogonality \citep{Dubovsky2012}]
\todo{Cite with page number (page 6).}
% Dubovsky2012: Definition 1.15, page 6
...
This notion of orthogonality was introduced
by \citet{Dubovsky2012}.
\todomaybe[inline]{Relate orthogonality to disjointness of essential sets. Don't forget to deal with DNF/CNF difference.}

For a given $f$,
we will call the size of its maximum orthogonal set
\definiendum{$\apply{ortho}{f}$}.

\begin{theorem}[$\apply{ortho}{f} \leq \apply{dnf}{f}$]
...
\todo{Is it actually equivalent? Is \quot{equivalent} a correct word, esp. since Cepek's coverability is in CNF (dual)?}
For brevity,
we will omit proof of the equivalence
\todomaybe{Write the proof. May require many definitions.}
...
We call such vectors \definiendum{switches} of $f$.
\end{definition}
