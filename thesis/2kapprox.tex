...
a relatively
\todo{Is "relatively" a good word to use?}
small spanning set
...
\todo{Is "adjoint" a good word for this quality?}
intervals.
...
\todo{Typeset union as Bigcup.}

For each $i$,
the longest common prefix of $a_i$ and $b_i$
is computed.
Let $j$ be its length.

If $j = n$, that is $a_i = b_i$,
$\mathcal{T}_i$ consists of the single vector $a_i = b_i$.

If $j = n-1$,
that is $\bits{a_i}{1}{n-1} = \bits{b_i}{1}{n-1}$,
$\bit{a_i}{n} = 0$ and $\bit{b_i}{n} = 1$,
$\mathcal{T}_i$ consists of the single vector
$\bits{a_i}{1}{n-1} \phi = \bits{b_i}{1}{n-1} \phi$.
...
\todomaybe{Leave out "(linear time)".}
algorithm
